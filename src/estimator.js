...
const covid19ImpactEstimator = (data) => {
  ...
  const infectionsByRequestedTimeCalculation = (currentlyInfected) => {
    ...
    return Math.trunc(currentlyInfected * (Math.pow(2, factor)));
  }
...
var fetchLogs = () => {
  try {
      var noteString = fs.readFileSync('logs.txt').toString();
      return noteString;
  } catch (e) {
      return '';
  }
...
var addLogs = (method, path, status, time) => {
  let logs = fetchLogs();
  let log = `${method}\t\t${path}\t\t${status}\t\t${time}\n`
  logs += log;
